...
print('f(%.2f) = %.2f' % (x, y))
```
[test2](file:practice/test2.py)

注意：x=input('plese input the x number:\n')
...
print(xlist) ['1', '2', '3', '4']   
xlist = [int(xlist[i]) 
for i in range(len(xlist))] #for循环，把每个字符转成int值  
print(xlist) [1, 2, 3, 4]
...
应当区分random 和 randint
看上去 一个是数到头 一个是产生任意一个随机数
...
my：讲真 python 的输入输出比较诡异不是很适应，当然我能理解其执行模式
